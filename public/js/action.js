...
$(function(){
   ...
   $(".contact").click(function(){ box_contact(); });
	/*退出管理*/
   $(".user_login_out").live('click',function(){if(confirm('确定退出管理?')){return true;}else{return false;}});
	/*收藏本站*/
    ...
    * */
	$(".ajax").live('click',function(){
		...
		if(tip!='' && tip!='undefined' && tip!=null){
			if(confirm(tip)){ PageAjax(url); }
		}else{
			PageAjax(url);
		}
		return false;
	});
	$('#user_center a').live('click',function(){
		var url = $(this).attr('href');
		var cmd = $(this).attr('cmd');
		var tip = $(this).attr('tip');
		
		if(cmd=='' || cmd=='undefined' || cmd==null){ cmd = 'ajax'; }
	...
	//加载页面头部登录信息
	$('#userloginbox').load('/global_v1/top_login_box');
	//绑定标签
	bindtip();
});
...
function PageMark(T)
{
	if(T==0)
	{
	  $('#user_center_box_mark').fadeOut(200);
	}else{
	  $('#mark_load_txt').text('载入中，请稍等...');
	  var h = $('#user_center_box').height();
	  var w = $('#user_center_box').width();
	  $('#user_center_box_mark').css({'height':h,'width':w}).fadeIn(0);;
	}

}
function PageAjax(toUrl)
{
	GAjax(toUrl,'user_center_box');
}
function GAjax(toUrl,toKey)
{
	PageMark(1);
	$("#" + toKey).load(toUrl,function(){ PageMark(0); });
}
