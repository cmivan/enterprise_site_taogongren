...
class Forget extends QT_Controller {
	...
	function __construct()
	{
		...
		//找回密码页面参数
		$this->step = is_num($this->session->userdata('get_step'));
		if($this->step==false){
	...
	function step_1()
	{
		...
		if($this->step==1){
		  ...
		  $get_code = is_num($this->session->userdata('forget_code'));
		  if(empty($get_mobile)||$get_code==false){ json_form_no('请先获取验证码!'); }
		  if($code!=$get_code){ json_form_no('验证码不正确!'); }
	...
	function step_2()
	{
		...
		if($this->step==2){
		  ...
		  $get_mobile = $this->session->userdata('forget_mobile');
		  if($get_mobile==''){ json_form_no('未找到你的手机号码!'); }
		  if($password==''){ json_form_no('请输入密码!'); }
		  if($password!=$password2){ json_form_no('两次输入的密码不一致哦!'); }
