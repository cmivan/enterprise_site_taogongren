...
class User extends QT_Controller {
	...
	function info($uid=0)
	{
	    ...
	    if(!empty($this->user)){
			...
			switch($this->user->classid)
			{
			 ...
			 case 2: #团队/企业
			    $unum = $this->db->query('select id from user where id='.$uid.' and uid=1')->num_rows();
				if($unum<=0){
	...
	{
		$is_login = is_num($this->logid);
		if($is_login){
			if($this->uid==$this->logid){ $this->data['is_teamer'] = true; }
			if($this->data['is_teamer']==false){
				//判断是否已加入群
				$rsnum = $this->User_Model->is_team_user($this->Tid,$this->logid);
				if($rsnum>0){
	...
	function user_worker()
	{
		...
		/*2012.2.14 For seo*/
		$goodat_industrys = $this->data["goodat_industrys"];
		$goodat_industrys = toText($goodat_industrys);
		if($goodat_industrys!=''){ $goodat_industrys.= ','; }
		$description = '';
		if(!empty($this->user)){ $description = cutstr(toText($this->user->note),50); }

		/*SEO设置*/
		$this->data['seo']['title']  = $goodat_industrys.'淘工人信息,全国装修工人大本营 淘工人网!';
		$this->data['seo']['keywords'] = $goodat_industrys.'招聘,求职,找装修工人,找室内设计师!';
		$this->data['seo']['description'] = $description.'全国各地最新的招聘、求职、招标等信息!高效对接的平台,通过海量的在线工人与即时的装修信息,帮业主省钱,帮工人赚钱,让天下没有难找的工人!';
	...
	function cases($id=0)
	{
		...
		if(empty($view)){ $view = $this->Case_Model->view($id,$this->uid,2); }
		if(empty($view)){ show_404('/index' ,'log_error'); }
	...
	function user_worker_left()
	{
		...
		$Tid = $this->Tid;
		//已创建
		if(is_num($Tid)){ $this->data["team_but"] = 1; }		
		/*评分类型*/
		$this->data["rating_class"] = $this->Rating_Model->rating_class(0);
		/*评分率*/
		$this->data["haoping_sroc"] = $this->Rating_Model->haoping_sroc($this->uid);
		/*好友信息*/
	...
	function user_worker_right()
	{
		...
		$this->data["goodat_industrys"] = '';
		if(!empty($goodat_industrys)){
		   $inum=0;
		   foreach($goodat_industrys as $gi_rs){ $inum++; 
			   $this->data["goodat_industrys"].= "<span>";
			   if($inum>1){$this->data["goodat_industrys"].= "、";}
			   $this->data["goodat_industrys"].= $gi_rs->title."</span>";
		   }
		}
		...
		$this->data["skills"]    = $this->Skills_Model->User_Prices($this->uid);        /*参考报价*/
