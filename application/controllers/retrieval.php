...
class Retrieval extends QT_Controller {
	...
	function index()
	{
		...
		//排序参数
		$this->data['o_cost'] = $this->input->getnum("o_cost",0);
		$this->data['o_visited'] = $this->input->getnum("o_visited",0);
		$this->data['o_endtime'] = $this->input->getnum("o_endtime",0);
	...
	function view($id=0)
	{
		...
		/*当前任务的城市或地区*/
		$c_id = get_num($view->c_id);
		$a_id = get_num($view->a_id);
		
		if($a_id)
		{
			$rs = $this->Place_Model->area($a_id);
			if(!empty($rs))
			{
				$this->data['where'] = $rs->a_name;
			}
		}
		elseif($c_id)
		{
			$rs = $this->Place_Model->city($c_id);
			if(!empty($rs))
			{
				$this->data['where'] = $rs->c_name;
			}
		}
		...
		/*评分类型*/
		$this->data['rating_class'] = $this->Evaluate_Model->rating_class(1);
		/*评分率*/
		$this->data['haoping_sroc'] = $this->Evaluate_Model->haoping_sroc($uid);
