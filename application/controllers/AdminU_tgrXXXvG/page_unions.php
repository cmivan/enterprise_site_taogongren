...
class Page_unions extends XT_Controller {
	...
	function index()
	{
		...
		if($del_id){ $this->Unions_Model->del($del_id); }
		//批量删除、数据处理
		$del_id = $this->input->post('del_id');
		if(!empty($del_id)){
			foreach($del_id as $delID){
				if(is_num($delID)){ $this->Unions_Model->del($delID); }
			}
		}
		...
		if($type_id==false){
			...
			$listsql = "select * from unions".$key_sql;
		}else{
			if(!empty($keyswordSql)){ $key_sql = " and ".$keyswordSql; }
			//筛选大类符合的
			$listsql = "select * from unions where type_id=$type_id".$key_sql;
		}
	...
	function type()
	{
		...
		if($go=='yes'){
			...
			$row = $this->db->query('select * from unions_type where t_id='.$type_id)->row();
			if(!empty($row)){
				//获取当前基本信息
				...
				//执行重新排序
				if($cmd=="up"){
					$row_up = $this->db->query('select * from unions_type where t_order_id>'.$at_t_order_id." order by t_order_id asc")->row();
					if(!empty($row_up)){
						$up_t_id = $row_up->t_id;
						$up_t_order_id = $row_up->t_order_id;
						$this->db->query("update unions_type set t_order_id=$at_t_order_id where t_id=$up_t_id");
						$this->db->query("update unions_type set t_order_id=$up_t_order_id where t_id=$at_t_id");
						json_form_yes('更新成功!');
					}else{
						json_form_no('排序已到上限!');
					}
				}elseif($cmd=="down"){
					$row_down = $this->db->query('select * from unions_type where t_order_id<'.$at_t_order_id." order by t_order_id desc")->row();
					if(!empty($row_down)){
						$down_t_id = $row_down->t_id;
						$down_t_order_id = $row_down->t_order_id;
						$this->db->query("update unions_type set t_order_id=$at_t_order_id where t_id=$down_t_id");
						$this->db->query("update unions_type set t_order_id=$down_t_order_id where t_id=$at_t_id");
						json_form_yes('更新成功!');
					}else{
						json_form_no('排序已到下限!');
					}
