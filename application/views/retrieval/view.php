...
<div class="tab_item"><table width="100%" border="0" cellpadding="2" cellspacing="0"><?php if(!empty($view_like)){?><?php foreach($view_like as $rs){?><tr><td><a title="<?php echo $rs->title?>" href="<?php echo site_url("retrieval/view/".$rs->id)?>"><?php echo cutstr($rs->title,18)?></a></td><td width="50" align="right"><?php echo $rs->id?>天前</td></tr><?php }}else{?><tr><td colspan="2">没找到相似的信息</td></tr><?php }?></table></div></div></div>
   ...
   <div class="content_box" id="retrieval_left_2"><div class="content"><div class="pic"> <?php foreach($view_img as $rs){?><li><a href="<?php echo site_url("retrieval/view/".$rs->id)?>"><img src="<?php echo img_retrieval($rs->pic)?>" height="100" /></a></li><?php }?></div><div class="clear"></div></div></div></div>
...
<div class="info"><table width="556" border="0" align="center" cellpadding="0" cellspacing="5"><tr><td width="47%" height="24">投标类型：<?php echo $this->User_Model->g_team_men($view->team_or_men)?></td><td width="53%">类别：<?php echo $this->Retrieval_Model->g_class($view->classid)?></td></tr>
  <tr><td height="24">预计费用：<?php echo $view->cost?> 元</td><td>工期：<?php echo dateDDD($view->job_stime)?> - <?php echo dateDDD($view->job_etime)?></td></tr><tr><td height="24" colspan="2">需求工种：<?php echo $this->Retrieval_Model->g_industrys($view->industryid)?></td></tr><tr>
...
if($sqlnum>3){$listyle=' style="display:none;"';}else{$listyle='';}?><li<?php echo $listyle?>><a href="<?php echo img_retrieval($rs->pic)?>" rel="lightbox[plants]" title="<?php echo $rs->note?>"><img src="<?php echo img_retrieval($rs->pic)?>" width="100" alt="" /></a></li><?php }}?><div class="clear"></div></div><div class="clear bottom_b"><div class="updown"><a href="javascript:void(0);" style="background:none; border:0;"><img src="views/images/search/arrow-up-black.gif" width="24" height="19" /></a></div></div><?php }?></div>
